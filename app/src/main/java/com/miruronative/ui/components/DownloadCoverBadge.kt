...
import com.miruronative.playback.EpisodeDownloadStage
import com.miruronative.playback.EpisodeDownloadUi

/**
 * The download state of an episode, drawn on top of its cover.
 *
 * Deliberately quiet when there is nothing happening: a finished episode gets a small corner tick
 * rather than a scrim, so a fully downloaded season still looks like artwork. Only work in
 * progress dims the cover, which is what makes an active download findable at a glance in a list
 * of a hundred.
 */
@Composable
...
/**
 * Determinate progress sweeps clockwise from the top; an unmeasurable stage spins the same ring as
 * a short arc, so the two read as one control rather than two different widgets.
 */
@Composable
